...
class ProviderReportService:
    ...
    def calculate_report(
        ...
        with raw_conn() as conn:
            ...
            # Top 20 accounts
            top_20 = None
            if provider_ids and periods:
                if len(provider_ids) == 1:
                    top_20 = self._calc_top_20_accounts(
                        cur, provider_ids[0], periods[-1][0], periods[-1][1], periods
                    )
                else:
                    top_20 = self._calc_top_20_accounts_multi(
                        cur, provider_ids, periods[-1][0], periods[-1][1], periods
                    )
    ...
    def _calc_top_20_accounts(
        ...
        """Calculate top 20 accounts by sum in last period for single provider."""
        cur.execute("""
            SELECT
                c.account_id,
    ...
    def _calc_top_20_accounts_multi(
        self,
        cur,
        ...
        """Calculate top 20 accounts by sum in last period for multiple providers."""
        placeholders = ','.join(['%s'] * len(provider_ids))
        cur.execute(f"""
            SELECT
                c.account_id,
            ...
            ORDER BY sum_last DESC
            LIMIT 20
        """, [last_period_from, last_period_to] + provider_ids)
            ...
            for period_from, period_to in reversed(all_periods):
                cur.execute(f"""
                    ...
                    WHERE c.account_id = %s
                      ...
                      AND c.period_to = %s
                      AND cp.provider_id IN ({placeholders})
                """, [account_id, period_from, period_to] + provider_ids)
                result = cur.fetchone()
                sum_val = float(result[0]) if result and result[0] else 0.0
